...
CHROME_OPTIONS = ['--headless', '--no-sandbox', '--ignore-certificate-errors',
...
class ScrapeAmazon:
    def __init__(self, URL):
        ...
        product_req = requests.get('/products/{}'.format(self.product_id))
        product_in_db = (int(product_req.status_code) == 200)
        if product_in_db:
            requests.put('/products/{}'.format(self.product_id),
                         json={"price": self.price})
        else:
            requests.post('/products/{}'.format(self.product_id), json={"name": self.name, "currency": self.currency,
                                                                        "old_price": self.old_price, "price": self.price, "availability": self.availability, "url": self.url, "img_url": self.img_url})
        requests.post('/product/{}'.format(self.product_id),
                      json={"price": self.price, "currency": self.currency})
    ...
    def scrape_parameter(self, driver, parameter):
        ...
        for selector in SELECTORS[self.website][parameter]['css']:
            try:
                return driver.find_element_by_css_selector(selector).get_attribute(attribute)
            except:
                pass
        return None
